...
class Referral(Base):
    ...
    # Связи
    new_user: Mapped["User"] = relationship("User", foreign_keys=[new_user_id], back_populates="referred_by")
    old_user: Mapped["User"] = relationship("User", foreign_keys=[old_user_id], back_populates="referred_users")
    
    # Связь с бонусами
    bonuses: Mapped[List["ReferralBonus"]] = relationship("ReferralBonus", back_populates="referral", cascade="all, delete-orphan")
...
class ReferralCode(Base):
    ...
    owner: Mapped["User"] = relationship("User", foreign_keys=[owner_id], back_populates="referral_codes")
...
class ReferralBonus(Base):
    ...
    bonus_type: Mapped[str] = mapped_column(String(50))
